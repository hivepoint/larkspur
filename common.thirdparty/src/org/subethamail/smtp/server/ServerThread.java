...
/**
 * ServerThread accepts TCP connections to the server socket and starts a new {@link Session} thread for each connection
 * which will handle the connection.
...
public class ServerThread extends Thread {
    ...
    private void runAcceptLoop() {
        while (!this.shuttingDown) {
            ...
            try {
                if (server.getSessionFactory() == null) {
                    session = new Session(server, this, socket);
                } else {
                    session = server.getSessionFactory().createSession(server, this, socket);
                }
            } catch (IOException e) {
                connectionPermits.release();
